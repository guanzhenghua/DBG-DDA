...
class TAHIN(nn.Module):
    ...
    def forward(self, g, user_key, item_key, user_idx, item_idx):
        ...
        print(user_emb)
        us = user_emb.detach().cpu().numpy()
        np.savetxt('user128.csv', us, delimiter=',')
        it = item_emb.detach().cpu().numpy()
        np.savetxt('item128.csv', it, delimiter=',')
        # obtain users/items embeddings and their interactions
        user_feat = user_emb[user_idx]
        item_feat = item_emb[item_idx]
        # interaction = user_feat @ item_feat.T

        # 创建 DataFrame 对象
        # us = user_feat.cpu().numpy()
        # np.savetxt('user64.csv', us, delimiter=',')
        #
        # it = item_feat.cpu().numpy()
        # # df = pd.DataFrame(v)  # 如果 user_feat 是 PyTorch 张量，需要先转换为 NumPy 数组
        #
        # # 将 DataFrame 存储为 CSV 文件
        # np.savetxt('item64.csv', it, delimiter=',')
